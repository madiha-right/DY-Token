    ...
    IRTokenAdmin,
{
    ...
    uint256 private _totalSupply; // Total number of tokens in circulation
    string private _name; // EIP-20 token name for this token
    string private _symbol; // EIP-20 token symbol for this token
    uint256 private _decimals; // EIP-20 token decimals for this token
    IERC20 private immutable _asset; // Underlying token
    uint256 public savingAssetOrignalAmount; // Saving assets original amount. This amount is in the same unit used in allocation strategy
    uint256 public savingAssetConversionRate; // Saving asset original to internal amount conversion rate
    mapping(address => mapping(address => uint256)) public transferAllowances; // Approved token transfer amounts on behalf of others
    Hat[] internal hats;
    ...
    function createHat(address[] calldata recipients, uint32[] calldata proportions, bool doChangeHat)
        external
        nonReentrant
    ...
    function getHatByAddress(address owner)
        external
        view
        returns (uint256 hatID, address[] memory recipients, uint32[] memory proportions)
    {
    ...
    function getGlobalStats() external view returns (GlobalStats memory) {
        ...
        return GlobalStats({totalSupply: totalSupply, totalSavingsAmount: totalSavingsAmount});
    }

    /// @dev IRToken.getAccountStats implementation
    function getAccountStats(address owner) external view returns (AccountStatsView memory stats) {
        Account storage account = accounts[owner];
    ...
    function getHatStats(uint256 hatID) external view returns (HatStatsView memory stats) {
    ...
    function getSavingAssetBalance() external view returns (uint256 rAmount, uint256 sOriginalAmount) {
    ...
    function changeAllocationStrategy(address allocationStrategy_) external nonReentrant onlyOwner {
        ...
        savingAssetConversionRate = sOriginalBurned.mul(savingAssetConversionRateOld).div(sOriginalCreated);
     ...
     */
    function transferInternal(address spender, address src, address dst, uint256 tokens) internal {
        ...
        if ((accounts[src].hatID != 0 && accounts[dst].hatID == 0 && accounts[src].hatID != SELF_HAT_ID)) {
     ...
     */
    function createHatInternal(address[] memory recipients, uint32[] memory proportions)
        internal
        returns (uint256 hatID)
    {
        uint256 i;
        ...
        for (i = 0; i < proportions.length; ++i) {
            proportions[i] = uint32(
                // don't panic, no safemath, look above comment
                (uint256(proportions[i]) * uint256(PROPORTION_BASE)) / totalProportions
            );
        }
     ...
     */
    function distributeLoans(address owner, uint256 rAmount, uint256 sInternalAmount) internal {
        Account storage account = accounts[owner];
        Hat storage hat = hats[account.hatID == SELF_HAT_ID ? 0 : account.hatID];
        uint256 i;
        if (hat.recipients.length > 0) {
            ...
            for (i = 0; i < hat.proportions.length; ++i) {
                ...
                uint256 lDebtRecipient = isLastRecipient ? rLeft : (rAmount.mul(hat.proportions[i])) / PROPORTION_BASE;
                ...
                recipientAccount.lDebt = recipientAccount.lDebt.add(lDebtRecipient);
                // remaining value adjustments
                rLeft = gentleSub(rLeft, lDebtRecipient);

                // calculate the savings holdings of the recipient
                uint256 sInternalAmountRecipient =
                    isLastRecipient ? sInternalLeft : (sInternalAmount.mul(hat.proportions[i])) / PROPORTION_BASE;
                recipientAccount.sInternalAmount = recipientAccount.sInternalAmount.add(sInternalAmountRecipient);
                // remaining value adjustments
                sInternalLeft = gentleSub(sInternalLeft, sInternalAmountRecipient);

                _updateLoanStats(
                    owner, hat.recipients[i], account.hatID, true, lDebtRecipient, sInternalAmountRecipient
                );
            }
        } else {
     ...
     */
    function estimateAndRecollectLoans(address owner, uint256 rAmount) internal returns (uint256 sInternalEstimated) {
    ...
    function recollectLoans(address owner, uint256 rAmount) internal {
        ...
        if (hat.recipients.length > 0) {
            ...
            for (i = 0; i < hat.proportions.length; ++i) {
                ...
                uint256 lDebtRecipient =
                    isLastRecipient ? rLeft : (debtToCollect.mul(hat.proportions[i])) / PROPORTION_BASE;
                recipientAccount.lDebt = gentleSub(recipientAccount.lDebt, lDebtRecipient);
                account.lRecipients[hat.recipients[i]] =
                    gentleSub(account.lRecipients[hat.recipients[i]], lDebtRecipient);
                // loans leftover adjustments
                rLeft = gentleSub(rLeft, lDebtRecipient);

                // calculate savings to be collected from the recipient
                uint256 sInternalAmountRecipient =
                    isLastRecipient ? sInternalLeft : (sInternalToCollect.mul(hat.proportions[i])) / PROPORTION_BASE;
                recipientAccount.sInternalAmount = gentleSub(recipientAccount.sInternalAmount, sInternalAmountRecipient);
                ...
                _updateLoanStats(
                    owner, hat.recipients[i], account.hatID, false, lDebtRecipient, sInternalAmountRecipient
                );
            }
        } else {
    ...
    function payInterestInternal(address owner) internal {
        ...
        if (interestAmount > 0) {
            stats.cumulativeInterest = stats.cumulativeInterest.add(interestAmount);
    ...
    function _updateLoanStats(
        ...
        emit LoansTransferred(owner, recipient, hatID, isDistribution, redeemableAmount, sInternalAmount);

        if (isDistribution) {
            hatStats.totalLoans = hatStats.totalLoans.add(redeemableAmount);
            hatStats.totalInternalSavings = hatStats.totalInternalSavings.add(sInternalAmount);
        } else {
            hatStats.totalLoans = gentleSub(hatStats.totalLoans, redeemableAmount);
            hatStats.totalInternalSavings = gentleSub(hatStats.totalInternalSavings, sInternalAmount);
        }
    }
    ...
    function sInternalToR(uint256 sInternalAmount) private view returns (uint256 rAmount) {
        ...
        //   =>
        return sInternalAmount.mul(ias.exchangeRateStored()).div(savingAssetConversionRate);
    }

    /// @dev convert redeemable amount to internal savings amount
    function rToSInternal(uint256 rAmount) private view returns (uint256 sInternalAmount) {
        return rAmount.mul(savingAssetConversionRate).div(ias.exchangeRateStored());
    }

    /// @dev convert original savings amount to redeemable amount
    function sOriginalToR(uint256 sOriginalAmount) private view returns (uint256 sInternalAmount) {
        return sOriginalAmount.mul(ias.exchangeRateStored()).div(ALLOCATION_STRATEGY_EXCHANGE_RATE_SCALE);
    }

    // @dev convert from original savings amount to internal savings amount
    function sOriginalToSInternal(uint256 sOriginalAmount) private view returns (uint256 sInternalAmount) {
        // savingAssetConversionRate is scaled by 1e18
        return sOriginalAmount.mul(savingAssetConversionRate).div(ALLOCATION_STRATEGY_EXCHANGE_RATE_SCALE);
    }

    // @dev convert from internal savings amount to original savings amount
    function sInternalToSOriginal(uint256 sInternalAmount) private view returns (uint256 sOriginalAmount) {
        // savingAssetConversionRate is scaled by 1e18
        return sInternalAmount.mul(ALLOCATION_STRATEGY_EXCHANGE_RATE_SCALE).div(savingAssetConversionRate);
    }
